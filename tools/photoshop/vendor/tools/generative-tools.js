import { clampGenerativeScale, ExtendScriptSnippets, normalizeExpandDirection, parseGenerativeResult, requireGenerativeCapability, runGenerativeSnippet, } from './generative/_shared.js';
function clampFeather(value) {
    const n = typeof value === 'number' ? value : Number(value);
    if (!Number.isFinite(n))
        return 0;
    return Math.max(0, Math.min(20, Math.round(n)));
}
export function createGenerativeTools(connection) {
    return [
        {
            tool: {
                name: 'generative_fill',
                description: 'Fill the current selection using Adobe Generative Fill (Firefly) with a text prompt.\n\n' +
                    'Use when: adding or replacing content inside a selection with generative AI.\n' +
                    'Do NOT use when: no selection exists — create one first or use select_subject.\n' +
                    'Do NOT use when: Photoshop version lacks generative_fill — check get_capabilities.\n\n' +
        ...
        },
        {
            tool: {
                ...
                description: 'Remove content using the AI Remove tool (or generative fill fallback) on the current selection.\n\n' +
                    'Use when: erasing distractions, people, or objects with generative AI.\n' +
                    'Do NOT use when: generative unavailable — fallback to recipe_remove_distraction.\n\n' +
        ...
        },
        {
            tool: {
                ...
                description: 'Extend the canvas beyond its edges using Generative Expand (Firefly).\n\n' +
        ...
        },
        {
            tool: {
                ...
                description: 'Upscale the active document using Generative Upscale (PS 27+).\n\n' +
                    'Use when: increasing resolution with AI detail recovery.\n' +
                    'Do NOT use when: generative_upscale flag is false — use resize_image.\n\n' +
                    'Returns: { ok, summary, details }.\n' +
        ...
        },
        {
            tool: {
                ...
                description: 'Replace the sky using Photoshop native Sky Replacement when available.\n\n' +
                    ...
                    'Fallback: recipe_sky_blend for manual composite.\n\n' +
                    'Returns: { ok, summary, details }.\n' +
        ...
        },
        {
            tool: {
                name: 'generate_image',
                description: 'Generate image content from a text prompt (text-to-image) on blank or active document.\n\n' +
...
async function generativeFill(connection, args) {
...
async function generativeRemove(connection, args) {
    const blocked = await requireGenerativeCapability(connection, 'generative_fill');
    if (blocked)
        return blocked;
    const feather = clampFeather(args.feather_px);
    const autoSelect = args.auto_select_subject === true;
    const raw = await runGenerativeSnippet(connection, ExtendScriptSnippets.generativeRemove(feather, autoSelect));
    return parseGenerativeResult(raw);
}
async function generativeExpand(connection, args) {
    const blocked = await requireGenerativeCapability(connection, 'generative_fill');
    if (blocked)
        return blocked;
    const prompt = typeof args.prompt === 'string' && args.prompt.trim()
    ...
    const raw = await runGenerativeSnippet(connection, ExtendScriptSnippets.generativeExpand(direction, prompt));
    return parseGenerativeResult(raw);
}
async function generativeUpscale(connection, args) {
    ...
    const raw = await runGenerativeSnippet(connection, ExtendScriptSnippets.generativeUpscale(scale));
    return parseGenerativeResult(raw);
}
async function skyReplacement(connection, args) {
    const blocked = await requireGenerativeCapability(connection, 'generative_fill');
    if (blocked)
        return blocked;
    const skyPath = typeof args.sky_image_path === 'string' ? args.sky_image_path.trim() : '';
    const raw = await runGenerativeSnippet(connection, ExtendScriptSnippets.skyReplacement(skyPath));
    return parseGenerativeResult(raw);
}
async function generateImage(connection, args) {
    const blocked = await requireGenerativeCapability(connection, 'generative_fill');
    if (blocked)
        return blocked;
    const prompt = typeof args.prompt === 'string' ? args.prompt.trim() : '';
    ...
    const height = typeof args.height === 'number' && Number.isFinite(args.height)
        ...
        : 1024;
    const raw = await runGenerativeSnippet(connection, ExtendScriptSnippets.generateImage(prompt, width, height));
    return parseGenerativeResult(raw);
}
