import { MCP_CURVES_ADJUSTMENT_HELPER, MCP_LAYER_MASK_HELPERS, } from '../../api/extendscript.js';
...
`;
export function wrapInSuspendHistory(historyName, body) {
 ...
 */
export function wrapInStandaloneScript(body) {
    return `
    ${RECIPE_ACTION_HELPERS}
    ${EXTENDSCRIPT_JSON_HELPER}
    var __mcp_recipe_result = null;
...
export function parseRecipeResult(raw) {
    ...
    if (rec.ok === true) {
        return {
            ...
            created_layer_ids: Array.isArray(rec.created_layer_ids)
                ? rec.created_layer_ids
                : undefined,
            output_paths: Array.isArray(rec.output_paths) ? rec.output_paths : undefined,
            next_suggested_tool: typeof rec.next_suggested_tool === 'string' ? rec.next_suggested_tool : undefined,
            undo_history_states_consumed: typeof rec.undo_history_states_consumed === 'number' ? rec.undo_history_states_consumed : 1,
            details: rec.details && typeof rec.details === 'object'
                ? rec.details
                : undefined,
        };
    }
    if (rec.ok === false) {
        return {
            ...
            suggested_next_tool: typeof rec.suggested_next_tool === 'string' ? rec.suggested_next_tool : undefined,
        };
    }
    return null;
}
...
export function toolFailure(result) {
    return {
        content: [{ type: 'text', text: JSON.stringify(result, null, 2) }],
        isError: true,
    };
}
export function toolException(error, code = 'recipe_exception') {
    const message = error instanceof Error ? error.message : String(error);
    return toolFailure({ ok: false, code, message });
}
export async function executeRecipe(connection, historyName, body) {
    try {
...
export async function executeStandaloneRecipe(connection, body) {
    try {
        const apiFactory = new PhotoshopAPIFactory(connection);
        const api = await apiFactory.createAPI();
...
export function gradientMaskAxisPercents(direction, startPct, endPct) {
...
export function gradientMaskDefaultAngle(direction) {
