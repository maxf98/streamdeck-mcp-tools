...
import { clampInt, executeRecipe, gradientMaskAxisPercents, jsString, toolFailure, } from './_shared.js';
const TOOL_NAME = 'recipe_sky_blend';
export function bindSkyBlend(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'One-shot sky composite: places an external sky image, adds a layer mask, and applies a horizon gradient fade. Wrapped in a single undoable history step.\n' +
                '\n' +
                ...
                'Do NOT use when: fading the active subject layer only — use recipe_gradient_fade.\n' +
                '\n' +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    use_native_sky: {
                        type: 'boolean',
                        description: 'Try native Sky Replacement first when supported (default true when capable)',
                    },
                },
...
async function runSkyBlend(connection, args) {
