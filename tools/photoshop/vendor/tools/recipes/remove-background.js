...
const TOOL_NAME = 'recipe_remove_background';
export function bindRemoveBackground(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'One-shot background removal: runs Select Subject on the active layer, inverts the selection, attaches a layer mask, and applies an optional feather. Wrapped in a single undoable history step.\n' +
                '\n' +
            ...
            inputSchema: {
                ...
                properties: {
                    feather_px: {
                        type: 'number',
                        description: 'Edge feather in pixels (0-20). 0 = hard edge (default for product shots), 1-3 = soft edge for portraits.',
                        minimum: 0,
                        maximum: 20,
                        default: 0,
                    },
                    keep_shadow: {
                        type: 'boolean',
                        description: 'Reserved for a future iteration; currently recorded in the response but no shadow layer is created yet.',
                        default: false,
                    },
                    use_generative: {
                        type: 'boolean',
                        description: 'After masking, run generative edge cleanup on inverted background selection (default false)',
                        default: false,
                    },
                },
            },
...
async function runRemoveBackground(connection, args) {
    ...
    if (layer.isBackgroundLayer) {
      return { ok: false, code: 'no_active_layer', message: 'Active layer is the Background layer — convert it to a normal layer first.', suggested_next_tool: 'rasterize_layer' };
    }
    ...
    if (!subjectSelected) {
      try {
        ...
        return { ok: false, code: 'generative_unavailable', message: 'Select Subject is not available: ' + (eSelectSubject.message || eSelectSubject), suggested_next_tool: 'get_capabilities' };
      }
    }
