...
const TOOL_NAME = 'recipe_batch_mockup_replace';
...
export function bindBatchMockupReplace(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: "Iterate a directory of asset images, replace the contents of the named Smart Object in the active mockup PSD for each asset, and export a flattened JPEG per variant. The mockup's perspective/warp on the Smart Object is preserved.\n" +
                '\n' +
                'Use when: the user has a mockup PSD and wants to render it once per design asset (logos, screens, product photos).\n' +
                'Do NOT use when: the asset is not a single layer (use place_image manually) or when the active document has no Smart Object with the requested name.\n' +
                '\n' +
            ...
            inputSchema: {
                ...
                properties: {
                    smart_object_layer_name: {
                        type: 'string',
                        description: 'Exact name of the Smart Object layer in the active document. Case-sensitive.',
                    },
                    assets_dir: {
                        type: 'string',
                        description: 'Absolute path to the directory containing asset files. Subdirectories are NOT recursed. Allowed extensions: jpg/jpeg/png/tif/tiff/psd/psb/webp.',
                    },
                    quality: {
                        type: 'number',
...
async function runBatchMockupReplace(connection, args) {
    const layerName = typeof args.smart_object_layer_name === 'string' ? args.smart_object_layer_name.trim() : '';
    ...
    if (!target) {
      return { ok: false, code: 'layer_not_found', message: 'Smart Object layer not found: ' + targetName, suggested_next_tool: 'get_layers' };
    }
    if (target.kind !== LayerKind.SMARTOBJECT) {
      return { ok: false, code: 'unsupported_color_mode', message: 'Target layer "' + targetName + '" is not a Smart Object (kind=' + target.kind + ').', suggested_next_tool: 'get_layers' };
    }
