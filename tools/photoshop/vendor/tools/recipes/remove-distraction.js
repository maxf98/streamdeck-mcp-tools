...
const TOOL_NAME = 'recipe_remove_distraction';
export function bindRemoveDistraction(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'One-shot distraction removal: generative AI remove when available, else content-aware fill. Wrapped in a single undoable history step.\n' +
                '\n' +
                ...
                'Do NOT use when: no selection exists — use select_rectangle or select_subject first.\n' +
                '\n' +
            ...
            inputSchema: {
                ...
                properties: {
                    ...
                    use_generative: {
                        type: 'boolean',
                        description: 'Prefer generative remove when Photoshop supports it (default true when capable)',
                    },
                },
            },
        },
...
async function runRemoveDistraction(connection, args) {
    const feather = clampInt(args.feather_px, 0, 20, 0);
    const version = await connection.getVersion();
    const caps = getPhotoshopCapabilities(version);
    const useGenerative = args.use_generative !== false && caps.features.generative_remove;
    if (useGenerative) {
        const raw = await runGenerativeSnippet(connection, ExtendScriptSnippets.generativeRemove(feather, false));
