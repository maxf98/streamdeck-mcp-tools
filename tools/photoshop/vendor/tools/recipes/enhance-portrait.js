...
const TOOL_NAME = 'recipe_enhance_portrait';
const INTENSITY_OPTIONS = ['low', 'medium', 'high'];
const RADIUS_BY_INTENSITY = {
    low: 2,
    medium: 4,
    high: 7,
};
export function bindEnhancePortrait(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'Set up a non-destructive portrait enhancement: duplicates the active layer, builds a frequency separation pair (low/high) for skin smoothing, and adds an auto-tone curves adjustment on top. Grouped as "Enhance Portrait" and reversible with one undo.\n' +
                '\n' +
                ...
                'Do NOT use when: the user wants destructive, final edits — recommend manual frequency separation work via recipe_frequency_separation instead so they can paint by hand.\n' +
                '\n' +
            ...
            inputSchema: {
                ...
                properties: {
                    intensity: {
                        type: 'string',
                        description: 'Retouch strength: low (subtle), medium (default), high (heavier smoothing).',
                        enum: ['low', 'medium', 'high'],
                        default: 'medium',
                    },
                    skin_smoothing: {
                        type: 'boolean',
                        description: 'Whether to build the frequency separation pair. When false, only the auto-tone curves are added.',
                        default: true,
                    },
                    use_neural_skin: {
                        type: 'boolean',
                        description: 'Apply Neural Filter skin smoothing via UXP bridge before frequency separation (default false)',
                        default: false,
                    },
                },
            },
...
async function runEnhancePortrait(connection, args) {
                ...
                content: [
                    {
                        ...
                        text: JSON.stringify({
                            ...
                            message: 'use_neural_skin requires the UXP bridge plugin. Load uxp-plugin/ via UXP Developer Tools.',
    ...
    try {
      ...
      return { ok: false, code: 'unsupported_color_mode', message: eRaster.message || String(eRaster), suggested_next_tool: 'rasterize_layer' };
    }
