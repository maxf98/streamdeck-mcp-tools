...
const TOOL_NAME = 'recipe_frequency_separation';
export function bindFrequencySeparation(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'Build a frequency separation stack (Low + High) on top of the active layer for hands-on retouching. Does not apply any smoothing itself — the user paints into the layers afterwards.\n' +
                '\n' +
                ...
                'Do NOT use when: the user wants a one-shot result without painting — use recipe_enhance_portrait instead.\n' +
                '\n' +
            ...
            inputSchema: {
                ...
                properties: {
                    radius_px: {
                        type: 'number',
                        description: 'Gaussian blur radius for the low-frequency layer. 4-8 for portraits, 10-20 for products. Default 6. Range 1-50.',
                        minimum: 1,
                        maximum: 50,
                        default: 6,
                    },
...
async function runFrequencySeparation(connection, args) {
