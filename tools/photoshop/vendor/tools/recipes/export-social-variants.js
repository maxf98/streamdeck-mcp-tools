...
const TOOL_NAME = 'recipe_export_social_variants';
...
export function bindExportSocialVariants(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'Render one JPEG per requested social-media platform from the active document. Each variant is center-cropped/resized to the platform spec and saved to disk.\n' +
                '\n' +
                'Use when: the user wants multi-platform deliverables in one shot.\n' +
                'Do NOT use when: only one export is needed (use recipe_prepare_for_web instead) or when platforms differ by content rather than crop (recipe does not change content, only frame).\n' +
                '\n' +
...
async function runExportSocialVariants(connection, args) {
    ...
    const variantsLiteral = variants
        .map((v) => `{ slug: "${jsString(v.slug)}", width: ${v.width}, height: ${v.height}, path: "${jsString(v.path)}" }`)
        .join(', ');
    const body = `
...
function parsePlatforms(raw) {
    ...
    if (Array.isArray(raw)) {
        slugs = raw.filter((s) => typeof s === 'string');
    }
