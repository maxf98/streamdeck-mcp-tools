...
const TOOL_NAME = 'recipe_apply_color_grade';
...
export function bindApplyColorGrade(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'Apply a named color grading preset as a non-destructive layer group (Hue/Saturation adjustment + brightness/contrast tweak).\n' +
                '\n' +
...
async function runApplyColorGrade(connection, args) {
    ...
    try {
      if (String(doc.mode) !== 'DocumentMode.RGB') {
        return { ok: false, code: 'unsupported_color_mode', message: 'Active document is not RGB; convert to RGB before applying a color grade.', suggested_next_tool: 'get_state' };
      }
    } catch (eMode) {}
