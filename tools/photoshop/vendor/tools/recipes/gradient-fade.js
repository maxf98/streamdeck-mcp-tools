import { clampInt, executeRecipe, gradientMaskAxisPercents, gradientMaskDefaultAngle, } from './_shared.js';
const TOOL_NAME = 'recipe_gradient_fade';
...
export function bindGradientFade(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: 'One-shot gradient fade on the active layer mask: creates a reveal-all mask if needed, then paints a linear black-to-white gradient for soft edge blending. Wrapped in a single undoable history step.\n' +
                '\n' +
                ...
                'Do NOT use when: the subject is not isolated — use recipe_remove_background first.\n' +
                'Do NOT use when: replacing the sky with an external image — use recipe_sky_blend.\n' +
                '\n' +
...
async function runGradientFade(connection, args) {
    ...
    const angle = typeof args.angle_deg === 'number' && Number.isFinite(args.angle_deg)
    ...
    if (!layer) {
      return { ok: false, code: 'no_active_layer', message: 'No active layer', suggested_next_tool: 'get_state' };
    }
...
function parseGradientDirection(value) {
    if (typeof value === 'string' && GRADIENT_DIRECTIONS.includes(value)) {
        return value;
    }
