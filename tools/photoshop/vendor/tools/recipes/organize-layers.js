...
const TOOL_NAME = 'recipe_organize_layers';
const NAMING_OPTIONS = ['type_index', 'content_summary', 'preserve'];
export function bindOrganizeLayers(connection) {
    return {
        tool: {
            name: TOOL_NAME,
            description: "Tidy the active document's layer stack: rename layers using a consistent scheme and optionally auto-group them by kind. Never deletes, merges or rasterizes — visual output stays identical.\n" +
                '\n' +
...
async function runOrganizeLayers(connection, args) {
    ...
    return {
      ...
      next_suggested_tool: 'get_layers',
      details: {
