 ...
 *
 * This pack needs NO macOS permissions, and that's a constraint to keep: the
 * pasteboard is not TCC-gated, so every tool here works on a clean machine with
 * no prompt. Reading the frontmost app's SELECTION is deliberately absent — the
 * only way to get it is to synthesize ⌘C (System Events keystroke or CGEventPost),
 * and both require Accessibility, which would make the whole pack permissioned for
 * one tool. Ask the user to copy first, then call get_clipboard.
 */
...
server.registerTool('set_clipboard', {
  ...
  description:
    ...
    'have (this pack cannot copy the user\'s current selection out of an app — ask them to ' +
    'press ⌘C, then read it with get_clipboard). ' +
