...
| `window_management` | macOS | Move and resize windows across screens, addressed by stable per-window handles; new windows land on the display you're at, not where the app cascaded them; live window list + app-switcher (key, dial and popup surfaces) as resources |
